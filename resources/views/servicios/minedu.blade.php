            ...
            <div class="d-flex flex-column border border-black rounded p-4" style="text-align: center">
                ...
                <div class="mb-4 my-0">
                    <form class="row g-3" method="POST" action="{{ route('registro') }}">
                        @csrf
                        <input type="hidden" name="servicio" value="MINEDU">
                        <input type="text" name="doc" class="form-control" placeholder="Ingrese Documento" pattern="^[^\s]+$" required maxlength="8">
                        <button type="submit" class="btn btn-success"><b>Buscar</b></button>
                        @if(isset($data) && isset($data['GetDataResponse']['GetDataResult']['DATA']['TituloContract']['DOCU_TIP']))
